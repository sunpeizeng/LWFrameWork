...
public class CreateScriptByTemplate
{
    ...
    {
        string path = EditorUtility.SaveFilePanel("新建ScriptableObject类", Application.dataPath + "/Scripts", "ScriptableObject", "cs");
        if (path != "")
        {
            ...
            if (!File.Exists(path))
            {
                ...
                TextAsset asset = Resources.Load<TextAsset>("Template/ScriptableObjectTemp");
                if (asset)
                {
            ...
            {
                Debug.LogError("新建Startup失败，已存在类型 " + className);
            }
        }
    }
    /// <summary>
    ...
    {
        string path = EditorUtility.SaveFilePanel("新建 Startup 类", Application.dataPath + "/Scripts", "Startup", "cs");
        if (path != "")
        {
            string className = path.Substring(path.LastIndexOf("/") + 1).Replace(".cs", "");
            if (!File.Exists(path))
            {
                ...
                TextAsset asset = Resources.Load<TextAsset>("Template/StartupTemp");
                if (asset)
                {
                    string code = asset.text;
                    code = code.Replace("#SCRIPTNAME#", className);
